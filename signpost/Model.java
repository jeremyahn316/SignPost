...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int a = 1; a < last + 1; a += 1) {
            for (int i = 0; i < _width; i += 1) {
                for (int j = 0; j < _height; j += 1) {
                    if (_solution[i][j] == a) {
                        checker.add(solution[i][j]);
                    }
                }
            }
        }
        if (checker.size() != last) {
        ...
        for (int i = 0; i < _width; i += 1) {
            for (int j = 0; j < _height; j += 1) {
                ...
                PlaceList newlst = _allSuccessors[i][j][square.direction()];
                square._successors = newlst;
                PlaceList p1 = square.successors();
                for (int loop = 0; loop < p1.size(); loop += 1) {
                    PlaceList temp = new PlaceList() { };
                    temp.add(square.pl);
                    if (get(p1.get(loop))._predecessors == null) {
                        get(p1.get(loop))._predecessors = temp;
                    } else {
                        get(p1.get(loop))._predecessors.add(square.pl);
                    }
                }
            }
        }
    ...
    Model(Model model) {
        ...
        for (Sq[] sqlst : this._board) {
            this._allSquares.addAll(Arrays.asList(sqlst));
        }

        for (Sq[] sqlst: this._board) {
            for (Sq sq: sqlst) {
                Sq head = model._board[sq.x][sq.y].head();
                Sq pre = model._board[sq.x][sq.y].predecessor();
                Sq succ = model._board[sq.x][sq.y].successor();

                if (pre == null) {
                    sq._predecessor = null;
                } else {
                    sq._predecessor = this.get(pre.x, pre.y);
                }

                if (succ == null) {
                    sq._successor = null;
                } else {
                    sq._successor = this.get(succ.x, succ.y);
                }

                sq._head = this.get(head.x, head.y);
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (int i = 0; i < width(); i += 1) {
            for (int j = 0; j < height(); j += 1) {
                Sq sq = _board[i][j];
                for (int loop = 0; loop < sq.successors().size(); loop += 1) {
                    if (sq.sequenceNum() != 0
                            && get(sq.successors().get(loop)).sequenceNum() != 0
                            && sq.connectable(get(sq.successors().get(loop)))) {
                        sq.connect(get(sq.successors().get(loop)));
                        counter += 1;
                    }
                }
            }
    ...
    void solve() {
        for (int i = 0; i < width(); i += 1) {
            for (int j = 0; j < height(); j += 1) {
                int seq = _solution[i][j];
                if (seq != width() * height()) {
                    Sq sq = solnNumToSq(seq);
                    Sq sq2 = solnNumToSq(seq + 1);
                    sq.connect(sq2);
                }
            }
        }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                releaseGroup(sgroup);
                int next = this.sequenceNum() + 1;
                Sq successor = this.successor();
                while (successor != null) {
                    successor._sequenceNum = next;
            ...
            if (s1.sequenceNum() != 0) {
                ...
                int before = s1.sequenceNum() - 1;
                Sq presuccessor = s1.predecessor();
                while (presuccessor != null) {
                    presuccessor._sequenceNum = before;
                    presuccessor = presuccessor.predecessor();
                    before--;
                }
            }

            Sq square = this.successor();
            while (square != null) {
                square._head = this.head();
        ...
        /** Helps disconnect as a helper method. */
        void helper() {
            if (!hasFixedNum()) {
                Sq temp = this.predecessor();
                while (temp != null && !temp.hasFixedNum()) {
                    temp = temp.predecessor();
                }
                if (temp == null) {
                    Sq another = this;
                    while (another != null) {
                        another._sequenceNum = 0;
                        another = another.predecessor();
                    }
                    if (this.predecessor() != null) {
                        this._group = newGroup();
                        Sq group = this.predecessor();
                        while (group != null) {
                            group._group = this._group;
        ...
        void disconnect() {
            ...
            _unconnected += 1;
            n._predecessor = _successor = null;
            if (_sequenceNum == 0) {
                if (successor() == null && predecessor() == null
                        && n.successor() == null && n.predecessor() == null) {
                    releaseGroup(this._group);
                    releaseGroup(n._group);
                    this._group = n._group = -1;
                } else if (n.predecessor() == null && n.successor() == null) {
                    n._group = -1;
                } else if (predecessor() == null && successor() == null) {
                    n._group = this._group;
                    this._group = -1;
                    n._head = n;
                } else {
                    n._group = newGroup();
                ...
                if (!n.hasFixedNum()) {
                    ...
                    if (temp == null) {
                        ...
                        while (another != null) {
                            another._sequenceNum = 0;
                            another = another.successor();
                        }
                        if (n.successor() != null) {
                            n._group = newGroup();
                            Sq group = n.successor();
                            while (group != null) {
                                group._group = n._group;
